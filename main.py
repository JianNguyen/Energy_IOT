...
def check_value():
    ...
    while wrong:
        ...
        for i in range(0, 5):
            power, flag = readdata()
            if flag == 1:
                all_value.append(power)
            time.sleep(1)
        print("All value: ",all_value)
        for i in range(1, len(all_value)):
            kt = all_value[i] - all_value[i - 1]
            if kt < 0 or kt >= 500:
                print("Read wrong data !")
                time_outt += 1
                flag = 0
                power = 0
                break
            else:
                if i == len(all_value) - 1:
                    flag = 1
                    power = all_value[-1]
                    wrong = False
        if time_outt >= 5:
...
def save_data_to_local(db, lock):
    ...
    while True:
        try:
            ...
            if (int(nowtime.strftime("%M")) < 10 or (
                    int(nowtime.strftime("%M")) >= 30 and int(nowtime.strftime("%M")) < 40)) and flag_read_data:
                print("---------------------------------------")
                print(nowtime)
                power, flag = check_value()
                if flag == 1:
                    lock.acquire()
            ...
            elif not (int(nowtime.strftime("%M")) < 10 or (
                    int(nowtime.strftime("%M")) >= 30 and int(nowtime.strftime("%M")) < 40)):
                flag_read_data = True
            time.sleep(10)
            ...
            if rs_data_local != None:
                if len(rs_data_local) > 0:
                    ...
                    for i in range(len(rs_data_local)):
                        rs_update = db.update_data_server(cabin_name, pi, group, rs_data_local[i][1],
                                                          rs_data_local[i][2])
                        if rs_update:
                            # lock.acquire()
